...
describe('composite handlers translate args correctly', () => {
 ...
 it('recent_calls with scope="domain" reads the domain CDR path, not the caller\'s own', async () => {
  // The office-manager bug: a domain-scoped question came back with only the
  // caller's own calls because every path went through /users/~/cdrs.
  const { handleToolCall } = await importTools();
  const calls: Array<{ pathTemplate: string; pathParams?: Record<string, string> }> = [];
  const fakeClient = {
   request: async (o: { pathTemplate: string; pathParams?: Record<string, string> }) => {
    calls.push(o);
    return { success: true, data: [] };
   },
  };
  await handleToolCall(fakeClient as never, 'recent_calls', { scope: 'domain' }, 'domain_admin');
  expect(calls[0].pathTemplate).toBe('/domains/{domain}/cdrs');
  expect(calls[0].pathTemplate).not.toContain('users');

  await handleToolCall(fakeClient as never, 'call_volume', { scope: 'domain' }, 'domain_admin');
  expect(calls[1].pathTemplate).toBe('/domains/{domain}/cdrs/count');
 });

 it('recent_calls stays user-scoped by default and when an explicit user is named', async () => {
  const { handleToolCall } = await importTools();
  const calls: Array<{ pathTemplate: string; pathParams?: Record<string, string> }> = [];
  const fakeClient = {
   request: async (o: { pathTemplate: string; pathParams?: Record<string, string> }) => {
  ...
  expect(calls[0].pathParams?.user).toBe('~');

  // An explicit user wins over scope="domain" — naming a user means that user.
  await handleToolCall(fakeClient as never, 'recent_calls', { scope: 'domain', user: '2001' }, 'domain_admin');
  expect(calls[1].pathTemplate).toBe('/domains/{domain}/users/{user}/cdrs');
